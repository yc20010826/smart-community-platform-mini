...
<!-- 				<view>共 {{commentData.commentSize}} 条评论</view> -->
				<view class="add-btn">
					<u-button type="primary" size="small" @click="commentInput(false)" icon="chat">发表评论</u-button>
				</view>
			</view>
				...
				<view class="comment-box-item">
					...
					<view class="comment-main">
						...
						<view class="comment-sub-box">
							<view class="comment-sub-item" v-for="(each,each_index) in item.children" :key="each_index">
								<view>
		...
		<view class="comment-submit-box" v-if="submit" @click="closeInput">
			...
			<view class="comment-add" @click.stop.prevent="stopPrevent" :style="'bottom:' + KeyboardHeight + 'px'" style="height: 250px;">
				<view class="comment-submit" style="height: 35px;">
					<view class="btn-click cancel" @click="closeInput" style="font-size: 14px;">取消</view>
					<view>
						<view class="replayTag" v-show="showTag" v-if="pUser">
							<view>回复在 {{pUser}} 的评论下</view>
							<view @click="tagClose" class="replyTagClose">×</view>
						</view>
					</view>
					<view>
						<view class="btn-click" @click="add" style="font-size: 14px;">发布</view>
					</view>
				</view>
